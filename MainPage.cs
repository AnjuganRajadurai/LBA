...
namespace LBA
{
    public partial class MainPage : Form
    {
        ...
        //Function that returns a boolean that tells if another Form is aleary openned
        private bool ifFormAlreadyOpened()
        {
            bool fromAlreadyOpened = false;

            if (Application.OpenForms.OfType<AddOperation>().Count() == 1)
            {
                fromAlreadyOpened = true;
            }
            else if (Application.OpenForms.OfType<AddPerson>().Count() == 1)
            {
                fromAlreadyOpened = true;
            }
            else if (Application.OpenForms.OfType<OperationHistory>().Count() == 1)
            {
                fromAlreadyOpened = true;
            }
            else if (Application.OpenForms.OfType<PersonProfil>().Count() == 1)
            {
                fromAlreadyOpened = true;
            }
            return fromAlreadyOpened;
        }
